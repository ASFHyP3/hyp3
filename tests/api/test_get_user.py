...
def test_user_at_quota(client, tables, monkeypatch):
    ...
    tables['job_table'].put_item(Item=make_db_record('anotherJob', request_time=request_time))
    response = client.get(USER_URI)
    assert response.status_code == status.HTTP_200_OK
    assert response.json['quota']['remaining'] == 0

    tables['job_table'].put_item(Item=make_db_record('yetAnotherJob', request_time=request_time))
    response = client.get(USER_URI)
    assert response.status_code == status.HTTP_200_OK
    assert response.json['quota']['remaining'] == 0
...
def test_get_user_custom_quota(client, tables):
    ...
    tables['user_table'].put_item(Item={'user_id': username, 'max_jobs_per_month': 50})
