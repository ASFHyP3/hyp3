...
def render_templates(job_types, security_environment, lifecycle_policy: bool):
...
def main():
    ...
    parser.add_argument('-l', '--lifecycle-policy', choices=['yes', 'no'])
    args = parser.parse_args()

    job_types = {}
    for file in args.job_spec_files:
        job_types.update(yaml.safe_load(file.read_text()))
    render_templates(job_types, args.security_environment, args.lifecycle_policy == 'yes')
